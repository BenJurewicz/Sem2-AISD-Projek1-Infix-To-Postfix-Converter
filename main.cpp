...
*/
/*
void parseSingleLine(Stack<Token> &postFixEquation, Stack<Token> &stack, int &count, bool isMainLoop) {
    static bool dotFound = false;
    char buff[12];
    int bracketCount = 0;
    //scanf("%s", buff);
    std::cin >> buff;
    while (!dotFound) {
        if (buff[0] == '.') {
            ...
            parseSingleLine(postFixEquation, stack, count, false);
            ...
            parseSingleLine(postFixEquation, stack, newCount, false);
